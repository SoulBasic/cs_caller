...
| 现象 | 快速定位 | 精确处理 |
| --- | --- | --- |
| 点击连接后未响应 | 状态长期停留“连接中...”，画面与输入可继续操作 | 1) 连接按钮在尝试期间会禁用，点“取消连接”立即中断并可重试；2) 超过约 3 秒会自动提示“连接超时”，不会卡死窗口；3) 优先确认 OBS NDI 输出已开启、源名与 GUI 输入一致；4) 若仍不稳定先切到 `mock` 验证流程，再回到 `ndi` 重连。 |
