...
| 现象 | 快速定位 | 精确处理 |
| --- | --- | --- |
| 点击连接后未响应 | 状态长期停留“连接中...”，但窗口可继续操作 | 1) `ndi` 模式握手在短生命周期子进程中执行，主 GUI 不会被阻塞；2) 超过约 3 秒会提示“NDI 握手超时（已终止子进程）”；3) 优先确认 OBS NDI 输出已开启、源名与 GUI 输入一致；4) 若仍不稳定先切到 `mock` 验证流程，再回到 `ndi` 重连。 |
